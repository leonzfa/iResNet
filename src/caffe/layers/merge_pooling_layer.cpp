...
void MergePoolingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void MergePoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  CHECK_EQ(4, bottom[0]->num_axes()) << "Input must have 4 axes, "
      << "corresponding to (num, channels, height, width)";
  // channels_, height_, width --> top[0]
  // pooled_height_, pooled_width --> bottom[0]

  channels_ = static_cast<int>(ceil(static_cast<float>( bottom[0]->channels() / 4))); //
  pooled_height_ = bottom[0]->height();
  pooled_width_  = bottom[0]->width();

  height_ = 2 * pooled_height_;
  width_  = 2 * pooled_width_;
...
// case?
template <typename Dtype>
void MergePoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void MergePoolingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(MergePoolingLayer);
#endif

INSTANTIATE_CLASS(MergePoolingLayer);
REGISTER_LAYER_CLASS(MergePooling);
}  // namespace caffe
