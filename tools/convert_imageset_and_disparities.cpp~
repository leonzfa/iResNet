...
public:
    ...
    ImagePair(std::string line)
    {
        clrptr();
        stringstream s(line);
        s >> img1_filename >> img2_filename >> disp_filename >> disp2_filename;
    }
    ~ImagePair()
    {
        if(data) delete data;
    ...
    void read_data()
    {
        ...
        if(has_disp())
        {
            ...
            cv_disp2 = ReadPFMImageToCVMat(disp2_filename, ySize, xSize, false);
        }
        ...
        data_size=  3*width*height              // Image 1
                   ...
                   +2*1*width*height            // disparity 1
                   +2*1*width*height;           // disparity 2
        ...
        for (int y = 0; y < height; ++y)
            for (int x = 0; x < width; ++x) {
                ...
                if(has_disp())
                {
                    if(isnan(cv_disp2.at<float>(y, x)))
                        value=std::numeric_limits<short>::max();
                    else
                        value=cv_disp2.at<float>(y, x)*32; 
                }
...
void debugData(string value)
{
    ...
    float* flow2=new float[xSize*ySize*2];
    float* occ2=new float[xSize*ySize];
