...
public:
    ...
    void read_data()
    {
        ...
        cv_img1 = cv::imread(img1_filename, CV_LOAD_IMAGE_GRAYSCALE);
        cv_img2 = cv::imread(img2_filename, CV_LOAD_IMAGE_GRAYSCALE);
        ...
        data_size=  width*height              // Image 1
        ...
        // Read image 1
            ...
            for (int y = 0; y < height; ++y)
                for (int x = 0; x < width; ++x) {
                    unsigned char value=cv_img1.at<uchar>(y, x);
                    *(ptr++)=value;
                }
        assert(ptr==data+1*width*height);

        // Read image 2
 
            for (int y = 0; y < height; ++y)
                for (int x = 0; x < width; ++x) {
                    unsigned char value=cv_img2.at<uchar>(y, x);
                    *(ptr++)=value;
                }
        assert(ptr==data+2*width*height);
...
int main(int argc, char** argv) {
  ...
  //for (int line_id = 0; line_id < 1; ++line_id) {
    ...
    snprintf(key_cstr, kMaxKeyLength, "%08d_%s", line_id,
        lines[line_id].img1_filename.c_str());
    string value;
